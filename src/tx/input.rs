...
impl SerializeTrait for Input {
    fn serialize(&self) -> Result<Vec<u8>, SerializationError> {
        ...
        let mut sg_size_varint = match VarInt::from_usize(self.scriptSig.len() as usize) {
            Ok(x) => x,
            Err(x) => return Err(SerializationError::VarIntErr(self.scriptSig.len() as usize))
        };
